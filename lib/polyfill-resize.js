...
DragResize.prototype.select = function(newElement) { with (this)
{
 ...
 if (newElement && (newElement != element) && enabled)
 {
  ...
  elmX = parseInt(style.left);
  elmY = parseInt(style.top);
  elmW = parseInt(style.width);
  elmH = parseInt(style.height);
...
DragResize.prototype.mouseMove = function(e) { with (this)
{
 ...
 element.style.cssText += "; left: " + (Math.round(elmX / gridX) * gridX) + "px; top: " + (Math.round(elmY / gridY) * gridY) + "px;";
 if (isResize)
 {
  element.style.cssText += "; width: " + (Math.round(elmW / gridX) * gridX) + "px; height: " + (Math.round(elmH / gridY) * gridY) + "px;";
 }
...
DragResize.prototype.resizeHandleSet = function(elm, show) { with (this)
{
 ...
 {
  if (elm['_handle_' + handles[h]]) return;
   var resizeHandleEl = elm.querySelector('.' + myName + '.' +  myName + '-' + handles[h]);
  if (resizeHandleEl)
  {
   elm['_handle_' + handles[h]] = resizeHandleEl;
  }
  else
  {
   // Create new divs, assign each a generic + specific class.
